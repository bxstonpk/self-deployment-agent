...
async def test_delete_application_never_deployed_deletes_directly():
    client = FakePlatformClient()
    idem = IdempotencyStore(ttl_seconds=60)
    app = await client.register_application("overtime", "desc", "dept-eng")
    # still "draft": never deployed, nothing to stop, so no archive step
...
async def test_delete_application_from_undeleteable_state_is_rejected():
    ...
    idem = IdempotencyStore(ttl_seconds=60)
    app = await client.register_application("overtime", "desc", "dept-eng")
    client.applications[app["id"]]["lifecycle_status"] = "deploying"  # mid-pipeline: neither
